...
struct k3_dsp_privdata {
	struct power_domain dsp_pwrdmn;
...
static int k3_dsp_load(struct udevice *dev, ulong addr, ulong size)
{
	...
	return ti_sci_proc_set_config(&dsp->tsp, boot_vector, 0, 0);
}
...
static int k3_dsp_start(struct udevice *dev)
{
	 ...
	 */
	ret = power_domain_on(&dsp->dsp_pwrdmn);
	if (ret) {
		dev_err(dev, "power_domain_on() failed: %d\n", ret);
		return ret;
	}

	ret = ti_sci_proc_release(&dsp->tsp);
	if (ret)
		return ret;
...
static int k3_dsp_stop(struct udevice *dev)
{
	...
	ret = power_domain_off(&dsp->dsp_pwrdmn);
	if (ret) {
		dev_err(dev, "power_domain_off() failed: %d\n", ret);
		return ret;
	}
...
static int k3_dsp_init(struct udevice *dev)
{
	...
	return ti_sci_proc_request(&dsp->tsp);
}
...
static int k3_dsp_reset(struct udevice *dev)
{
	...
	return ti_sci_proc_release(&dsp->tsp);
}
...
static int k3_dsp_of_get_memories(struct udevice *dev)
{
	...
	dsp->mem = calloc(num_mems, sizeof(*dsp->mem));
	if (!dsp->mem)
		return -ENOMEM;
	...
	for (i = internal_mems; i < num_mems; i++) {
		...
		dsp->mem[i].cpu_addr = map_physmem(dsp->mem[i].bus_addr,
						   ...
						   MAP_NOCACHE);
		/*
		 * Right now DDR map is same for DSP and Rest of the SoC.
		 * If MMU mapping is done, the address translation should
		 * come from image.
		 */
		dsp->mem[i].dev_addr = dsp->mem[i].bus_addr;

		dev_dbg(dev, "memory bus addr %pa size 0x%zx va %p da %pa\n",
			&dsp->mem[i].bus_addr, dsp->mem[i].size,
			dsp->mem[i].cpu_addr, &dsp->mem[i].dev_addr);
	}
	dsp->num_mems = num_mems;
...
static int k3_dsp_of_to_priv(struct udevice *dev, struct k3_dsp_privdata *dsp)
{
	...
	ret = power_domain_get(dev, &dsp->dsp_pwrdmn);
	if (ret) {
